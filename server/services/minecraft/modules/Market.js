module.exports = function Market() {
  ...
  server.market = {
    getMarketItemStats(username, item) {
      server.StatDb.find({ item: item }, { limit: 15 })
        .then((items) => {
          if (items) {
    ...
    getMarketItemsFrom(username, receiver, filter) {
      const _filter = {}
      server.ItemDb.find({ username: username, market: true }, _filter)
        .then((items) => {
          if (items) {
    ...
    getMarketItemsBy(username, item, filter) {
      const _filter = {}
      server.ItemDb.find({ item: item, market: true }, _filter)
        .then((items) => {
          if (items) {
            server.io.to(username).emit('init_market', items)
            console.info(
    ...
    setMarketItem(username, name, amount, limit) {
      ...
      if (amount) {
        ...
        server.ItemDb.findOneAndUpdate(
          ...
          { $set: { market: true, price: price, limit } },
          { new: true }
        ).then((item) => {
    ...
    buyMarketItem(username, name, seller, amount, price) {
      ...
      server.UserDb.findOne({ username: username }).then((user) => {
        server.ItemDb.findOne({ item: name, username: seller }).then((item) => {
          ...
          const _limit = item.limit ? item.limit : 0
          if (item.amount < amount) {
            _amount = item.amount
            _price = _amount * price
          }
          if (user.xp >= _price && item.amount >= (_amount + _limit)) {
