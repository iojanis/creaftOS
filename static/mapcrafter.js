...
PosHashHandler.prototype.gotoHash = function (hash) {
  ...
  if (!(hash[0] in this.ui.getMapConfigs()) ||
			this.ui.getMapConfig(hash[0]).rotations.indexOf(hash[1]) < 0) { return null }
...
function createMCTileLayer(mapName, mapConfig, mapRotation) {
  return new MCTileLayer('https://map.CraftOS/' + mapName + '/' + ['tl', 'tr', 'br', 'bl'][mapRotation], {
...
MapcrafterUI.prototype.setMap = function (map) {
  ...
  if (sameWorld && mapConfig.rotations.indexOf(this.currentRotation) != -1) {
    ...
    if (mapConfig.rotations.indexOf(rotation) == -1) { rotation = mapConfig.rotations[0] }
