<template>
  <transition name="slideRight">
    <div
      v-if="$store.state.UserDrawer"
      class="ui right notification-bar vertical inverted sidebar visible menu"
      style="z-index: 105"
    >
      <div v-if="$auth.loggedIn" style="padding: 0.5em">
        <span class="ui green inverted basic label noselect">
          <a
            ...
            style="margin-left: 0"
            ><i class="arrow down icon" style="margin: 0" @click="downloadExp"
          /></a>
          <span v-else class="detail" style="margin-left: 0"
            ><i class="arrow down icon disabled" style="margin: 0"
          /></span>
          {{ currentExp }}°
          <a
            v-if="isCurrentlyOnline"
            ...
            style="margin-left: 0"
            ><i class="arrow up icon" style="margin: 0" @click="uploadExp"
          /></a>
          <span v-else class="detail" style="margin-left: 0"
            ><i class="arrow up icon disabled" style="margin: 0"
          /></span>
        </span>
        <nuxt-link :to="'/whois/' + $auth.user.username">
          <img
            :src="'https://minotar.net/avatar/' + $auth.user.username"
            :class="{ online: isCurrentlyOnline }"
            style="
              height: 30px;
              width: 30px;
              border-radius: 3px;
...
export default {
  ...
  methods: {
    ...mapActions(["downloadExp", "uploadExp", "toggleUserDrawer", "toggleQuickUpload"]),
    async logOut() {
